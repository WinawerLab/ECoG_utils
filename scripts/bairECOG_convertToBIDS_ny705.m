...
task_label  = {'boldhand',...
               ...
               'boldsat3', ... 
               'boldsat4'}; %
run_label = {'01','01','01','01','01','01','01'}; % run labels separated by session
...
if length(dataFiles) > 1, disp('Warning: multiple datafiles found'); end
if length(dataFiles) < 1, disp('Warning: no data file(s) found!'); return; end
...
badChannels = [180 173 148 128 127 106 104 97 94 78 75 50 33]; 
...
badChannelsDescriptions = repmat({'spikes'}, size(badChannels));
    %{'clipped', 'clipped', ...
...
[~,trigger_onsets] = findpeaks(triggers, hdr.Fs, 'MinPeakHeight', 0.3, 'MinPeakDistance', 0.1);
...
switch makePlots 
    case 'yes'
        ...
        plot(t,triggers);
        plot(trigger_onsets, zeros(length(trigger_onsets),1),'r.','MarkerSize', 25, 'LineStyle','none');
...
stimMatFiles = dir(fullfile(stimDir, sprintf('sub-*%d*ses-%s*.mat', patientID, ses_label)));
stimTSVFiles = dir(fullfile(stimDir, sprintf('sub-*%d*ses-%s*.tsv', patientID,ses_label)));
...
for ii = 1:length(stimMatFiles)
    ...
    switch makePlots 
        case 'yes'
            ...
            title(stimData(ii).params.experiment)
    end
end
...
% TEMP HACK FOR HIGH DENSITY GRIDS %
for cChan = 1:length(chanNames)
    if contains(chanNames{cChan}, 'GA') || contains(chanNames{cChan}, 'GB')
        chanTypes{cChan} = 'ecog';
    end 
end
...
for ii = 1:nRuns
    ...
    % HACK Overwrite n/a in events_table (to prevent problems with
    % preprocessing)
    if ischar(events_table.stim_file_index(1))
        events_table.stim_file_index = repmat(1, [height(events_table) 1]);
    end
