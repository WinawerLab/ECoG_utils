...
% Input paths specs
patientID   = 718; % Specify patient's raw folder name here
...
% For 718, there's a mismatch for one set of electrodes that are labeled
% 'DAMT' and 'DPMT' in the data, but 'AMT' and 'PMT' in the electrode
% coordinates provided by SoM. Hack by overwriting names in the hdr:
INX = find(contains(hdr.label, {'DAMT', 'DPMT', 'DSL'})); 
for ii = 1:length(INX)
    oldlabel = hdr.label{INX(ii)};
    newlabel = oldlabel(2:end);
...
BADCHANNELS_MANUALTABLE = {...
    16, 'low freq';
    17, 'low freq';
    42, 'on epileptic tissue';
    43, 'on lesion';
    44, 'on lesion';
    51, 'on lesion';
    52, 'on lesion';
    46, 'on epileptic tissue';
    73, 'spikes';
    81, 'spikes';
    87, 'spikes';
    88, 'low freq';
